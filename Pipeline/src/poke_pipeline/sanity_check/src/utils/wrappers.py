...
class PartialObservabilityWrapper(gym.ObservationWrapper):
    """
    Wrapper that reduces observation space for partial observability testing.
    
    Specifically designed for Acrobot-v1 to remove velocity information
    (θ̇₁, θ̇₂) from observations, testing agent memory capabilities.
    """
    
    def __init__(self, env: gym.Env, hidden_indices: Optional[list] = None):
        """
        Initialize the wrapper.
        
        Args:
            env: The environment to wrap (should be Acrobot-v1)
            hidden_indices: List of observation indices to hide.
                           For Acrobot-v1: [4, 5] to hide velocity components
        """
        super().__init__(env)
        
        # Default to Acrobot-v1 velocity hiding if no indices specified
        if hidden_indices is None:
            if 'Acrobot' in str(env.spec.id):
                hidden_indices = [4, 5]  # Hide θ̇₁, θ̇₂ (velocity components)
            else:
                raise ValueError("hidden_indices must be specified for non-Acrobot environments")
        
        self.hidden_indices = hidden_indices
        
        # Validate original observation space
        if not isinstance(env.observation_space, Box):
            raise ValueError(f"PartialObservabilityWrapper requires Box observation space, got {type(env.observation_space)}")
        
        # Create new observation space with reduced dimensions
        original_low = env.observation_space.low
        original_high = env.observation_space.high
        original_shape = env.observation_space.shape
        
        # Remove hidden dimensions
        visible_indices = [i for i in range(original_shape[0]) if i not in hidden_indices]
        
        new_low = original_low[visible_indices]
        new_high = original_high[visible_indices]
        ...
        print(f"🔬 PartialObservabilityWrapper: {env.spec.id}")
        print(f"   Original obs shape: {original_shape}")
        print(f"   New obs shape: {self.observation_space.shape}")
        print(f"   Hidden indices: {hidden_indices}")
    
    def observation(self, obs: np.ndarray) -> np.ndarray:
        """
        Remove hidden components from observation.
        
        Args:
            obs: Original observation from environment
            
        Returns:
            Reduced observation with hidden components removed
        """
        return obs[self.visible_indices]
...
class DictObsWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, env: gym.Env, discrete: bool = False):
        ...
        if discrete:
            ...
            # For continuous environments like CartPole, LunarLander
