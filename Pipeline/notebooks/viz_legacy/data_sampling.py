...
class StreamingDataSampler:
    ...
    def _process_file_streaming(self, file_path: Path) -> Tuple[int, int]:
        """
        Process a single statistics file in a streaming fashion. Supports JSON arrays
        and the CSV format produced by Pipeline.
        ...
        if suffix == ".csv":
            return self._process_csv_streaming(file_path)
        ...
        try:
            ...
            self._print(f"Error processing {file_path.name}: {e}")
            return 0, 0
    ...
    def _process_csv_streaming(self, csv_file: Path) -> Tuple[int, int]:
        """
        Stream a CSV stats file using pandas chunking.
        """
        total_entries = 0
        processed_entries = 0
        ...
        try:
            for chunk in pd.read_csv(csv_file, chunksize=5000):
                ...
                for entry in records:
                    ...
                    normalized_entry = self._normalize_entry_format(entry, self.current_variant)
                    self._reservoir_sample(normalized_entry)
                    self._update_statistics(step_val)
                    processed_entries += 1
    ...
    def _normalize_entry_format(self, entry: dict, variant_name: str) -> dict:
        ...
        """
        # Normalize dotted keys (e.g. CSV columns)
    ...
    def load_variant_data(self, experiment_path: str, variant_name: str,
        ...
        for i, stat_file in enumerate(tqdm(stat_files, desc=f"Streaming {variant_name}")):
            try:
                ...
                file_entries, processed_entries = self._process_file_streaming(stat_file)
...
def load_variants_for_comparison(variant_configs: List[Tuple[str, str]],
                                 ...
                                 verbose: bool = True) -> Tuple[List[pd.DataFrame], TrainingSampler]:
    """
