...
class BaseRedGymEnv(Env, ABC):
    ...
    def __init__(self, config=None):
        ...
        self.debug_release_tick_render_off = bool(config.get("debug_release_tick_render_off", False))
        override_timing = config.get("debug_action_timing_override")
        self.debug_action_timing_override = override_timing if isinstance(override_timing, dict) else None
        ...
        self.log_stats_to_file = bool(config.get("log_stats_to_file", False))
        self.stats_flush_freq = int(config.get("stats_flush_freq", 200))
        self._stats_buffer: list[dict[str, Any]] = []
        self._stats_file = None
        if self.log_stats_to_file:
            logs_dir = self.s_path / "logs"
            logs_dir.mkdir(exist_ok=True)
            stats_path = logs_dir / f"worker_{self.worker_rank:02}_stats.jsonl"
            self._stats_file = stats_path.open("a", encoding="utf-8")
    ...
    def step(self, action):
        ...
        debug_mode = self.debug_reset_timing
        if debug_mode:
            self._log_reset_timing(
                f"step_phase action_start reset={self.reset_count} step_count={self.step_count} action={int(action)}"
            )
            action_ts = time.perf_counter()
        self.run_action_on_emulator(action)
        if debug_mode:
            self._log_reset_timing(
                f"step_phase action_end reset={self.reset_count} step_count={self.step_count} action={int(action)} "
                f"dt={time.perf_counter() - action_ts:.6f}"
            )
        ...
        render_start = time.perf_counter() if debug_mode else None
        current_screen = self.render()
        render_duration = time.perf_counter() - render_start if debug_mode else None
        self.update_recent_screens(current_screen)
        if debug_mode:
            self._log_reset_timing(
                f"step_phase render_end reset={self.reset_count} step_count={self.step_count} "
                f"dt_render={render_duration:.6f}"
            )

        # Update seen coords and exploration
        explore_ts = time.perf_counter() if debug_mode else None
        self.update_seen_coords()
        self.update_explore_map()
        if debug_mode:
            self._log_reset_timing(
                f"step_phase explore_end reset={self.reset_count} step_count={self.step_count} "
                f"dt_explore={time.perf_counter() - explore_ts:.6f}"
            )

        # Update heal reward and death tracking
        heal_ts = time.perf_counter() if debug_mode else None
        self.update_heal_reward()
        self.party_size = self.read_m(0xD163)
        if debug_mode:
            self._log_reset_timing(
                f"step_phase heal_end reset={self.reset_count} step_count={self.step_count} "
                f"dt_heal={time.perf_counter() - heal_ts:.6f}"
            )

        # Update rewards
        reward_ts = time.perf_counter() if debug_mode else None
        self.update_reward()
        if debug_mode:
            self._log_reset_timing(
                f"step_phase reward_end reset={self.reset_count} step_count={self.step_count} "
                f"dt_reward={time.perf_counter() - reward_ts:.6f}"
            )
    ...
    def run_action_on_emulator(self, action):
        ...
        debug_mode = self.debug_reset_timing
        overall_start = time.perf_counter() if debug_mode else None
        if debug_mode:
            self._log_reset_timing(
        ...
        # Press phase
        press_start = time.perf_counter() if debug_mode else None
        if debug_mode:
            self._log_reset_timing(
                f"action_phase press_tick_start reset={self.reset_count} step_count={self.step_count} action={action}"
            )
        press_ticks = press_step
        if getattr(self, "debug_action_timing_override", None):
            press_ticks = max(1, int(self.debug_action_timing_override.get("press_ticks", press_ticks)))
        self.pyboy.tick(press_ticks, render_screen)
        if debug_mode:
            self._log_reset_timing(
                f"action_phase press_tick_end reset={self.reset_count} step_count={self.step_count} "
                f"dt={time.perf_counter() - press_start:.6f}"
            )

        # Release phase
        release_start = time.perf_counter() if debug_mode else None
        self.pyboy.send_input(self.release_actions[action])
        if debug_mode:
            self._log_reset_timing(
                f"action_phase release_tick_start reset={self.reset_count} step_count={self.step_count}"
            )
        release_ticks = self.act_freq - press_ticks - 1
        release_render = render_screen and not self.debug_release_tick_render_off
        if getattr(self, "debug_action_timing_override", None):
            release_ticks = max(0, int(self.debug_action_timing_override.get("release_ticks", release_ticks)))
        if release_ticks > 0:
            self.pyboy.tick(release_ticks, release_render)
        if debug_mode:
            self._log_reset_timing(
                f"action_phase release_tick_end reset={self.reset_count} step_count={self.step_count} "
                f"dt={time.perf_counter() - release_start:.6f}"
            )

        # Final sync tick
        final_tick_start = time.perf_counter() if debug_mode else None
        final_render = True and not self.debug_release_tick_render_off
        if getattr(self, "debug_action_timing_override", None):
            final_render = final_render and bool(self.debug_action_timing_override.get("final_render", True))
            final_ticks = int(self.debug_action_timing_override.get("final_ticks", 1))
        else:
            final_ticks = 1
        if final_ticks > 0:
            self.pyboy.tick(final_ticks, final_render)
        if debug_mode:
            self._log_reset_timing(
                f"action_phase final_tick_end reset={self.reset_count} step_count={self.step_count} "
                f"dt={time.perf_counter() - final_tick_start:.6f}"
            )
        ...
        if debug_mode:
            ...
            if total_dt >= 0.5:
                self._log_reset_timing(
                    f"action_watchdog reset={self.reset_count} step_count={self.step_count} "
                    f"action={action} dt={total_dt:.6f}"
                )
            self._log_reset_timing(
                f"action_end reset={self.reset_count} step_count={self.step_count} dt_total={total_dt:.6f}"
            )
    ...
    def append_agent_stats(self, action):
        ...
        if self.log_stats_to_file:
            ...
            if len(self._stats_buffer) >= self.stats_flush_freq:
    ...
    def flush_stats_buffer(self):
        """Flush buffered stats to per-worker file."""
        if not self.log_stats_to_file or not self._stats_buffer or not self._stats_file:
            return
        records = len(self._stats_buffer)
        start_ts = time.perf_counter()
        for record in self._stats_buffer:
            json.dump(record, self._stats_file)
        ...
        if self.debug_reset_timing:
            ...
            if duration >= 0.05:
                self._log_reset_timing(
                    f"stats_flush reset={self.reset_count} step_count={self.step_count} "
                    f"records_flushed={records} dt={duration:.6f}"
                )
