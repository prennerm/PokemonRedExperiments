...
class RedGymEnv(Env):
    ...
    def render(self, reduce_res=True, add_memory=True, update_mem=True):
        ...
        if reduce_res:
            ...
            if update_mem:
                self.recent_frame = game_pixels_render
    ...
    def step(self, action):
        ...
        obs_memory = self.render(reduce_res=True, add_memory=False, update_mem=False)

        if self.use_screen_explore:
            self.update_frame_knn_index(obs_memory.flatten().astype(np.float32))
        else:
    ...
    def add_video_frame(self):
        ...
        self.model_frame_writer.add_image(self.render(reduce_res=True, add_memory=False, update_mem=False))
    ...
    #hier wurden create_exploration_memory und create_recent_memory
    ...
    def save_and_print_info(self, done, obs_memory):
        if self.print_rewards:
            ...
            for key, val in self.progress_reward.items():
                if key != 'explore':  # Entferne den 'explore' Reward
        ...
        if self.step_count % 50 == 0:
            plt.imsave(
                ...
                self.render(reduce_res=False, add_memory=False))
